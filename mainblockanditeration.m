...
for n1=[8]
    for it1=[1,3,5,10]
        ...
        %    save('temp1','q','result','plotcolor','lname')
           ...
           if q>1
               lname{int}=['IFDMA with q=' num2str(q) ',n=' num2str(n) ',it=' num2str(itnum) ',block=' num2str(b1)];
           end
           int=int+1;
            save('temp1','q','b1','n1','it1','int','result','plotcolor','lname')
                clear all
                load temp1
        end     
    end
    ...
    grid on
    title('BER of distributed IFDMA for different no of block and iteration')
    saveas(gcf,'graph it and block','jpg')
    legend(lname)